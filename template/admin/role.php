<?
	if(getID()) {
		if (file_exists(__ABSPATH__.'/template/admin/roleID.php')) {
			require_once(__ABSPATH__.'/template/admin/roleID.php');
		}
		die();
	}
...
if (empty($_POST)) {
    ...
    if ($form->isGroupValid()) :	
			global $db;
			$db->query('INSERT INTO Role(Role_ID,Role_Name,Role_Discription) VALUE(NULL,"'.$form->getValue('Name').'","'.$form->getValue('Discription').'")');
...
<?get_header()?>
<?global $db;?>
<?$rows = $db->get_results('SELECT * FROM Role',ARRAY_A);?>
<?=(isset($status)) ? $status : '' ;?>
...
<?if(isset($rows)):?>
	<table class="table table-striped table-hover">
		...
		<?foreach($rows as $row):?>
			<tr>
				<th><?=$row['Role_ID']?></th>
				<th><a href="<?=__BASEURL__.getModule().'/'.getAction().'/'.$row['Role_ID'].'/'?>"><?=$row['Role_Name']?></a></th>
				<th><?=$row['Role_Discription']?></th>
			</tr>
		<?endforeach;?>
		</tbody>
	</table>
