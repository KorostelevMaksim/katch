...
if (empty($_POST)) {
    ...
    if ($form->isGroupValid()) :	
			global $db;
			$db->query('INSERT INTO Controller(Controller_ID,Controller_Name,Controller_Discription) VALUE(NULL,"'.$form->getValue('Name').'","'.$form->getValue('Discription').'")');
...
<?get_header()?>
<?global $db;?>
<?$rows = $db->get_results('SELECT * FROM Controller',ARRAY_A);?>
<?=(isset($status)) ? $status : '' ;?>
