	...
	*/
	...
	class ezSQLcore
	{
		...
		*/
		...
		function q($from, $type = 'SELECT', $item = '*', $where = NULL,$order = NULL, $output = ARRAY_A) {
				...
				if($type == 'ODKU') {
					...
					if(is_array($item)){
						...
						foreach ($item as $key => $value) {
							...
							$odku .= " {$key}=";
							$odku .= (is_int($value)) ? $value.',' : "'{$value}',";
						}
						$query = substr_replace($query, '',-1,1);
						$val = substr_replace($val, '',-1,1);
						$odku = substr_replace($odku, '',-1,1);
						$query .= ') VALUES '.$val.') ON DUPLICATE KEY UPDATE'.$odku;
					}
					...
					if (is_array($item)) {
						...
						$array = substr_replace($array, '',-1,1);
						$query .= " {$array}";
					} else {
						$query .= " {$item}";
					}
					...
					if(is_array($from)) {
						$query .= " FROM";
						foreach ($from as $table => $letter) {
							$query .= " {$table} AS {$letter},";
						}
						$query = substr_replace($query, '',-1,1);
					} else {
						$query .= " FROM {$from}";
				...
				if (isset($where)) {
					if (is_array($where)) {
						...
						foreach ($where as $key=>$value):
								$array .= " {$key}={$value} AND";
						endforeach;
						$array = substr_replace($array, '',-3,3);
					}
					$query .= " WHERE {$array}";
				}
